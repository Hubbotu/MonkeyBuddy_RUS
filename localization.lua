-- Not translations
MONKEYBUDDY_TITLE							= "MonkeyBuddy";
MONKEYBUDDY_VERSION							= "2.11.0";
MONKEYBUDDY_FRAME_TITLE						= MONKEYBUDDY_TITLE .. " v" .. MONKEYBUDDY_VERSION;
MONKEYBUDDY_QUEST_TITLE						= "MonkeyQuest";
MONKEYBUDDY_SPEED_TITLE						= "MonkeySpeed";
MONKEYBUDDY_CLOCK_TITLE						= "MonkeyClock";
MONKEYBUDDY_INFO_COLOUR						= "|cffffff00";
MONKEYBUDDY_GUI_ICON						= "\124TInterface\\AddOns\\MonkeyLibrary\\Textures\\MonkeyBuddyIcon.tga:0\124t "
BINDING_HEADER_MONKEYBUDDY					= MONKEYBUDDY_TITLE;

-- English, the default
MONKEYBUDDY_DESCRIPTION						= "Helps you configure your MonkeyMods.";
MONKEYBUDDY_LOADED_MSG						= MONKEYBUDDY_INFO_COLOUR .. MONKEYBUDDY_TITLE .. " v" .. MONKEYBUDDY_VERSION .. " loaded";

MONKEYBUDDY_TOOLTIP_CLOSE					= "Close";
MONKEYBUDDY_RESET_ALL						= "Reset All";
MONKEYBUDDY_RESET_TO_BLIZZARD_STYLE			= "Blizzard Style";
MONKEYBUDDY_RESET							= "Reset";

MONKEYBUDDY_GUI_TEXT						="This addon allows you to easily configure your MonkeyMods"
MONKEYBUDDY_GUI_MBLOADED					= MONKEYBUDDY_GUI_ICON .. "MonkeyBuddy successfully loaded!"
MONKEYBUDDY_GUI_MMINSTALLED					="Installed MonkeyMods:"
MONKEYBUDDY_GUI_MBMINIMAP					="Show the MonkeyBuddy minimap icon"
MONKEYBUDDY_GUI_MQEXTRA						="MonkeyQuest Extra:"
MONKEYBUDDY_GUI_MQEDAILY					="Show the daily quest counter in the title"
MONKEYBUDDY_GUI_MBOPENCFG					="Click on the following Button to open"

-- defs for MonkeyQuest
MONKEYBUDDY_QUEST_OPEN						= "Open MonkeyQuest";
MONKEYBUDDY_QUEST_SHOWHIDDEN				= "Show hidden items";
MONKEYBUDDY_QUEST_USEOVERVIEWS				= "Use overviews when there's no objectives";
MONKEYBUDDY_QUEST_HIDEHEADERS				= "Hide zone headers if not showing hidden items";
MONKEYBUDDY_QUEST_ALWAYSHEADERS				= "Always show zone headers, always";
MONKEYBUDDY_QUEST_HIDEBORDER				= "Hide the border";
MONKEYBUDDY_QUEST_GROWUP					= "Expand upwards";
MONKEYBUDDY_QUEST_SHOWNUMQUESTS				= "Show the number of quests";
MONKEYBUDDY_QUEST_LOCK						= "Lock the MonkeyQuest frame";
MONKEYBUDDY_QUEST_COLOURTITLEON				= "Colour the quest titles by difficulty";
MONKEYBUDDY_QUEST_HIDECOMPLETEDQUESTS		= "Hide completed quests";
MONKEYBUDDY_QUEST_HIDECOMPLETEDOBJECTIVES	= "Hide completed objectives";
MONKEYBUDDY_QUEST_SHOWTOOLTIPOBJECTIVES		= "Show objective completeness in tooltips";
MONKEYBUDDY_QUEST_ALLOWRIGHTCLICK			= "Allow right click to open MonkeyBuddy";
MONKEYBUDDY_QUEST_HIDETITLEBUTTONS			= "Hide the title buttons";
MONKEYBUDDY_QUEST_HIDETITLE					= "Hide the title (" .. MONKEYBUDDY_QUEST_TITLE .. ") text";
MONKEYBUDDY_QUEST_FONT						= "Font:";
MONKEYBUDDY_QUEST_CRASHBORDER				= "Use the golden border colour";
MONKEYBUDDY_QUEST_SHOWNOOBTIPS				= "Show helpful tooltips for Noobs";
MONKEYBUDDY_QUEST_SHOWZONEHIGHLIGHT			= "Show quest zone highlighting";
MONKEYBUDDY_QUEST_SHOWQUESTLEVEL			= "Show the quest levels";
MONKEYBUDDY_QUEST_WORKCOMPLETE				= "Enable >work complete< sound";
MONKEYBUDDY_QUEST_COLOURSUBOBJECTIVES		= "Colour objectives by progress";
MONKEYBUDDY_QUEST_ITEMSENABLED				= "Show quest item buttons (Tainted in 6.0.3)"
MONKEYBUDDY_QUEST_ITEMSONLEFT				= "Show quest items on left side";
MONKEYBUDDY_QUEST_HIDEHEADER				= "Only show title/titlebuttons on mouseover"
MONKEYBUDDY_QUEST_HIDEQUESTSENABLED			= "Hide quests via right click if not showing hidden items"
MONKEYBUDDY_QUEST_COLOURDONEORFAILED		= "Colour finished or failed quest titles"
MONKEYBUDDY_QUEST_QUESTTEXTTOOLTIP			= "Show full quest text on mouse over"

MONKEYBUDDY_QUEST_QUESTTITLECOLOUR			= "Quest Titles";
MONKEYBUDDY_QUEST_HEADEROPENCOLOUR			= "Open Zone Headers";
MONKEYBUDDY_QUEST_HEADERCLOSEDCOLOUR		= "Closed Zone Headers";
MONKEYBUDDY_QUEST_OVERVIEWCOLOUR			= "Quest Overviews";
MONKEYBUDDY_QUEST_SPECIALOBJECTIVECOLOUR	= "Special Objectives";
MONKEYBUDDY_QUEST_INITIALOBJECTIVECOLOUR	= "Objectives at 0%";
MONKEYBUDDY_QUEST_MIDOBJECTIVECOLOUR		= "Objectives at 50%";
MONKEYBUDDY_QUEST_COMPLETEOBJECTIVECOLOUR	= "Objectives at <100%";
MONKEYBUDDY_QUEST_FINISHOBJECTIVECOLOUR		= "Finished Objectives";
MONKEYBUDDY_QUEST_ZONEHIGHLIGHTCOLOUR		= "Current Zone Highlight";

MONKEYBUDDY_QUEST_FRAMEALPHASLIDER			= "Global Alpha";
MONKEYBUDDY_QUEST_ALPHASLIDER				= "Background Alpha";
MONKEYBUDDY_QUEST_HIGHLIGHTALPHASLIDER		= "Zone Highlight Alpha";
MONKEYBUDDY_QUEST_WIDTHSLIDER				= "Frame Width";
MONKEYBUDDY_QUEST_FONTSLIDER				= "Font Size";
MONKEYBUDDY_QUEST_PADDINGSLIDER				= "Quest Padding";

-- defs for MonkeySpeed
MONKEYBUDDY_SPEED_OPEN						= "Open MonkeySpeed";
MONKEYBUDDY_SPEED_PERCENT					= "Show speed as a percent";
MONKEYBUDDY_SPEED_BAR						= "Show speed as a background colour";
MONKEYBUDDY_SPEED_LOCK						= "Lock the MonkeySpeed frame";
MONKEYBUDDY_SPEED_ALLOWRIGHTCLICK			= "Allow right click to open MonkeyBuddy";

MONKEYBUDDY_SPEED_WIDTHSLIDER				= "Frame Width";
MONKEYBUDDY_SPEED_HEIGHTSLIDER				= "Frame Height";

-- defs for MonkeyClock
MONKEYBUDDY_CLOCK_OPEN						= "Open MonkeyClock";
MONKEYBUDDY_CLOCK_HIDEBORDER				= "Hide the border";
MONKEYBUDDY_CLOCK_USEMILITARYTIME			= "Use 24 hour clock";
MONKEYBUDDY_CLOCK_LOCK						= "Lock the MonkeyClock frame";
MONKEYBUDDY_CLOCK_CHATALARM					= "Use the alarm message in the chat window";
MONKEYBUDDY_CLOCK_DIALOGALARM				= "Use the alarm dialog box with snooze button";
MONKEYBUDDY_CLOCK_ALLOWRIGHTCLICK			= "Allow right click to open MonkeyBuddy";

MONKEYBUDDY_CLOCK_HOURSLIDER				= "Hour Offset";
MONKEYBUDDY_CLOCK_MINUTESLIDER				= "Minute Offset";

MONKEYBUDDY_CLOCK_ALARMHOURSLIDER			= "Alarm Hour";
MONKEYBUDDY_CLOCK_ALARMMINUTESLIDER			= "Alarm Minute";

-- bindings
BINDING_NAME_MONKEYBUDDY_OPEN				= "Open/Close the config frame";

if (GetLocale() == "deDE") then

MONKEYBUDDY_DESCRIPTION						= "Hilft Euch beim Einstellen Euer MonkeyMods.";
MONKEYBUDDY_LOADED_MSG						= MONKEYBUDDY_INFO_COLOUR .. MONKEYBUDDY_TITLE .. " v" .. MONKEYBUDDY_VERSION .. " geladen";

MONKEYBUDDY_TOOLTIP_CLOSE					= "Schlie\195\159en";
MONKEYBUDDY_RESET_ALL						= "Alles Zur\195\188cksetzen";
MONKEYBUDDY_RESET							= "Zur\195\188cksetzen";

MONKEYBUDDY_GUI_TEXT						="Dieses AddOn hilft euch beim Einstellen euer MonkeyMods"
MONKEYBUDDY_GUI_MBLOADED					= MONKEYBUDDY_GUI_ICON .. "MonkeyBuddy erfolgreich geladen!"
MONKEYBUDDY_GUI_MMINSTALLED					="Installierte MonkeyMods:"
MONKEYBUDDY_GUI_MBMINIMAP					="Zeige das MonkeyBuddy MiniMap Symbol"
MONKEYBUDDY_GUI_MQEXTRA						="MonkeyQuest Extra:"
MONKEYBUDDY_GUI_MQEDAILY					="Zeige die Anzahl der t\195\164glichen Quests im Titel"
MONKEYBUDDY_GUI_MBOPENCFG					="Ein Klick auf folgenden Button \195\182ffnet"

-- defs for MonkeyQuest
MONKEYBUDDY_QUEST_OPEN						= "\195\150ffne MonkeyQuest";
MONKEYBUDDY_QUEST_SHOWHIDDEN				= "Zeige ausgeblendete Elemente";
MONKEYBUDDY_QUEST_USEOVERVIEWS				= "Benutze Questanweisungen, wenn keine Ziele vorhanden sind";
MONKEYBUDDY_QUEST_HIDEHEADERS				= "Verberge Questzonen, wenn ausgeblendete Elemente versteckt werden";
MONKEYBUDDY_QUEST_ALWAYSHEADERS				= "Zeige die Questzonen immer";
MONKEYBUDDY_QUEST_HIDEBORDER				= "Verberge den Rahmen";
MONKEYBUDDY_QUEST_GROWUP					= "Erweitere MonkeyQuest nach oben hin";
MONKEYBUDDY_QUEST_SHOWNUMQUESTS				= "Zeige die Anzahl der Quests";
MONKEYBUDDY_QUEST_LOCK						= "Fixiere das MonkeyQuest Fenster";
MONKEYBUDDY_QUEST_COLOURTITLEON				= "F\195\164rbe die Quests der Schwierigkeit ensprechend ein";
MONKEYBUDDY_QUEST_HIDECOMPLETEDQUESTS		= "Verberge erf\195\188llte Quests";
MONKEYBUDDY_QUEST_HIDECOMPLETEDOBJECTIVES	= "Verberge erf\195\188llte Ziele";
MONKEYBUDDY_QUEST_SHOWTOOLTIPOBJECTIVES		= "Zeige Zielfortschritte in Tooltips";
MONKEYBUDDY_QUEST_ALLOWRIGHTCLICK			= "\195\150ffnen von MonkeyBuddy durch Rechtsklick erm\195\182glichen";
MONKEYBUDDY_QUEST_HIDETITLEBUTTONS			= "Verberge die Titel-Buttons";
MONKEYBUDDY_QUEST_HIDETITLE					= "Verberge den Text (" .. MONKEYBUDDY_QUEST_TITLE .. ") im Titel";
MONKEYBUDDY_QUEST_FONT						= "Schriftart:";
MONKEYBUDDY_QUEST_CRASHBORDER				= "Benutze die goldene Rahmen-Farbe";
MONKEYBUDDY_QUEST_SHOWNOOBTIPS				= "Zeige hilfreiche Kurzinfos f\195\188r Anf\195\164nger";
MONKEYBUDDY_QUEST_SHOWZONEHIGHLIGHT			= "Hebe die aktuelle Questzone hervor";
MONKEYBUDDY_QUEST_SHOWQUESTLEVEL			= "Zeige das Questlevel";
MONKEYBUDDY_QUEST_WORKCOMPLETE				= "Aktiviere >Arbeit erledigt< Sound";

MONKEYBUDDY_QUEST_QUESTTITLECOLOUR			= "Quest-Titel";
MONKEYBUDDY_QUEST_HEADEROPENCOLOUR			= "Offene Zonen";
MONKEYBUDDY_QUEST_HEADERCLOSEDCOLOUR		= "Geschlossene Zonen";
MONKEYBUDDY_QUEST_OVERVIEWCOLOUR			= "Questanweisungen";
MONKEYBUDDY_QUEST_SPECIALOBJECTIVECOLOUR	= "Spezielle Ziele";
MONKEYBUDDY_QUEST_INITIALOBJECTIVECOLOUR	= "Ziele zu 0% erf\195\188llt";
MONKEYBUDDY_QUEST_MIDOBJECTIVECOLOUR		= "Ziele zu 50% erf\195\188llt";
MONKEYBUDDY_QUEST_COMPLETEOBJECTIVECOLOUR	= "Ziele zu <100% erf\195\188llt";
MONKEYBUDDY_QUEST_FINISHOBJECTIVECOLOUR		= "Komplett erledigte Ziele";
MONKEYBUDDY_QUEST_ZONEHIGHLIGHTCOLOUR		= "Questzonen-Hervorhebung";

MONKEYBUDDY_QUEST_FRAMEALPHASLIDER			= "Global Alpha";
MONKEYBUDDY_QUEST_ALPHASLIDER				= "Hintergrund Alpha";
MONKEYBUDDY_QUEST_HIGHLIGHTALPHASLIDER		= "Questzonen-Hervorhebung Alpha"
MONKEYBUDDY_QUEST_WIDTHSLIDER				= "Fensterbreite";
MONKEYBUDDY_QUEST_FONTSLIDER				= "Schriftgr\195\182\195\159e";
MONKEYBUDDY_QUEST_PADDINGSLIDER				= "Zeilenabstand";

-- defs for MonkeySpeed
MONKEYBUDDY_SPEED_OPEN						= "\195\150ffne MonkeySpeed";
MONKEYBUDDY_SPEED_PERCENT					= "Zeige Geschwindigkeit in Prozent";
MONKEYBUDDY_SPEED_BAR						= "Zeige Geschwindigkeit als farbigen Hintergrund";
MONKEYBUDDY_SPEED_LOCK						= "Fixiere das MonkeySpeed Fenster";
MONKEYBUDDY_SPEED_ALLOWRIGHTCLICK			= "\195\150ffnen von MonkeyBuddy durch Rechtsklick erm\195\182glichen";

MONKEYBUDDY_SPEED_WIDTHSLIDER				= "Fensterbreite";
MONKEYBUDDY_SPEED_HEIGHTSLIDER				= "Fensterh\195\182he";

-- defs for MonkeyClock
MONKEYBUDDY_CLOCK_OPEN						= "\195\150ffne MonkeyClock";
MONKEYBUDDY_CLOCK_HIDEBORDER				= "Verberge den Rahmen";
MONKEYBUDDY_CLOCK_USEMILITARYTIME			= "Benutze den 24-Stundentakt";
MONKEYBUDDY_CLOCK_LOCK						= "Fixiere das MonkeyClock Fenster";
MONKEYBUDDY_CLOCK_CHATALARM					= "Aktviere die Alarm-Mitteilung im Nachrichtenfenster";
MONKEYBUDDY_CLOCK_DIALOGALARM				= "Aktiviere die Alarm-Mitteilung als PopUp mit Verl\195\164ngerungsoption";
MONKEYBUDDY_CLOCK_ALLOWRIGHTCLICK			= "\195\150ffnen von MonkeyBuddy durch Rechtsklick erm\195\182glichen";

MONKEYBUDDY_CLOCK_HOURSLIDER				= "Stundenausgleich";
MONKEYBUDDY_CLOCK_MINUTESLIDER				= "Minutenausgleich";

MONKEYBUDDY_CLOCK_ALARMHOURSLIDER			= "Alarmstunde";
MONKEYBUDDY_CLOCK_ALARMMINUTESLIDER			= "Alarmminute";

-- bindings
BINDING_NAME_MONKEYBUDDY_OPEN				= "\195\150ffne/Schlie\195\159e das Optionsfenster";

end

if ( GetLocale() == "frFR" ) then

MONKEYBUDDY_DESCRIPTION						= "Vous aide Ã  configurer vos MonkeyMods.";
MONKEYBUDDY_LOADED_MSG						= MONKEYBUDDY_INFO_COLOUR .. MONKEYBUDDY_TITLE .. " v" .. MONKEYBUDDY_VERSION .. " chargÃ©";

MONKEYBUDDY_TOOLTIP_CLOSE					= "Fermer";
MONKEYBUDDY_RESET_ALL						= "R.\195\160.Z. Totale";
MONKEYBUDDY_RESET							= "R.\195\160.Z.";

--MONKEYBUDDY_GUI_TEXT						="This addon allows you to easily configure your MonkeyMods"
--MONKEYBUDDY_GUI_MBLOADED					= MONKEYBUDDY_GUI_ICON .. "MonkeyBuddy successfully loaded!"
--MONKEYBUDDY_GUI_MMINSTALLED					="Installed MonkeyMods:"
--MONKEYBUDDY_GUI_MBMINIMAP					="Show the MonkeyBuddy minimap icon"
--MONKEYBUDDY_GUI_MQEXTRA						="MonkeyQuest Extra:"
--MONKEYBUDDY_GUI_MQEDAILY					="Show the daily quest counter in the title"
--MONKEYBUDDY_GUI_MBOPENCFG					="Click on the following Button to open"

-- defs for MonkeyQuest
MONKEYBUDDY_QUEST_OPEN						= "Ouvrir MonkeyQuest";
MONKEYBUDDY_QUEST_SHOWHIDDEN				= "Montrer toutes les qu\195\170tes";
MONKEYBUDDY_QUEST_USEOVERVIEWS				= "Utiliser la description lorsqu\'il n\'y a pas d\'objectifs";
MONKEYBUDDY_QUEST_HIDEHEADERS				= "Cacher les noms de zone";
--MONKEYBUDDY_QUEST_ALWAYSHEADERS				= "Always show zone headers, always";
MONKEYBUDDY_QUEST_HIDEBORDER				= "Cacher les bords";
MONKEYBUDDY_QUEST_GROWUP					= "Augmenter la fen\195\170tre vers le haut";
MONKEYBUDDY_QUEST_SHOWNUMQUESTS				= "Montrer le nombre de qu\195\170tes";
MONKEYBUDDY_QUEST_LOCK						= "Bloquer la fen\195\170tre MonkeyQuest";
MONKEYBUDDY_QUEST_COLOURTITLEON				= "Colorer les titres de qu\195\170tes selon la difficult\195\169";
MONKEYBUDDY_QUEST_HIDECOMPLETEDQUESTS		= "Cacher les qu\195\170tes termin\195\169es";
MONKEYBUDDY_QUEST_HIDECOMPLETEDOBJECTIVES	= "Cacher les objectifs termin\195\169s";
MONKEYBUDDY_QUEST_SHOWTOOLTIPOBJECTIVES		= "Afficher les objectifs dans la bulle d\'aide";
MONKEYBUDDY_QUEST_ALLOWRIGHTCLICK			= "Clic droit pour ouvrir MonkeyBuddy";
MONKEYBUDDY_QUEST_HIDETITLEBUTTONS			= "Cacher les boutons de titre";
MONKEYBUDDY_QUEST_HIDETITLE					= "Cacher le texte (" .. MONKEYBUDDY_QUEST_TITLE .. ") dans le titre";
MONKEYBUDDY_QUEST_FONT						= "La police:";
MONKEYBUDDY_QUEST_CRASHBORDER				= "Utiliser la couleur de bordure de Crash";
MONKEYBUDDY_QUEST_SHOWNOOBTIPS				= "Afficher les bulles d\'aide pour les Noobs";
--MONKEYBUDDY_QUEST_SHOWZONEHIGHLIGHT			= "Show quest zone highlighting";
--MONKEYBUDDY_QUEST_SHOWQUESTLEVEL			= "Show the quest levels";
--MONKEYBUDDY_QUEST_WORKCOMPLETE				= "Enable >work complete< sound";

MONKEYBUDDY_QUEST_QUESTTITLECOLOUR			= "Titre des Qu\195\170tes";
MONKEYBUDDY_QUEST_HEADEROPENCOLOUR			= "Ouvrir le nom des zones";
MONKEYBUDDY_QUEST_HEADERCLOSEDCOLOUR		= "Fermer le nom des zones";
MONKEYBUDDY_QUEST_OVERVIEWCOLOUR			= "Vues d\'ensemble des Qu\195\170tes";
MONKEYBUDDY_QUEST_SPECIALOBJECTIVECOLOUR	= "Objectifs Spéciaux";
MONKEYBUDDY_QUEST_INITIALOBJECTIVECOLOUR	= "Objectifs \195\160 0%";
MONKEYBUDDY_QUEST_MIDOBJECTIVECOLOUR		= "Objectifs \195\160 50%";
MONKEYBUDDY_QUEST_COMPLETEOBJECTIVECOLOUR	= "Objectifs \195\160 <100%";
MONKEYBUDDY_QUEST_FINISHOBJECTIVECOLOUR		= "Objectifs \195\160 100%";
MONKEYBUDDY_QUEST_ZONEHIGHLIGHTCOLOUR		= "Surligner la Zone courrante";

MONKEYBUDDY_QUEST_FRAMEALPHASLIDER			= "Transparence globale";
MONKEYBUDDY_QUEST_ALPHASLIDER				= "Transparence du fond";
--MONKEYBUDDY_QUEST_HIGHLIGHTALPHASLIDER	= "Zone Highlight Alpha";
MONKEYBUDDY_QUEST_WIDTHSLIDER				= "Largeur de la fen\195\170tre";
MONKEYBUDDY_QUEST_FONTSLIDER				= "Taille de la Police";
MONKEYBUDDY_QUEST_PADDINGSLIDER				= "Quest Padding";

-- defs for MonkeySpeed
MONKEYBUDDY_SPEED_OPEN						= "Ouvrir MonkeySpeed";
MONKEYBUDDY_SPEED_PERCENT					= "Montrer la vitesse comme un pourcentage";
MONKEYBUDDY_SPEED_BAR						= "Montrer la vitesse comme une couleur de fond";
--MONKEYBUDDY_SPEED_LOCK						= "Lock the MonkeySpeed frame";
MONKEYBUDDY_SPEED_ALLOWRIGHTCLICK			= "Clic droit pour ouvrir MonkeyBuddy";

--MONKEYBUDDY_SPEED_WIDTHSLIDER				= "Frame Width";
--MONKEYBUDDY_SPEED_HEIGHTSLIDER				= "Frame Height";

-- defs for MonkeyClock
MONKEYBUDDY_CLOCK_OPEN						= "Ouvrir MonkeyClock";
MONKEYBUDDY_CLOCK_HIDEBORDER				= "Cacher les bords";
MONKEYBUDDY_CLOCK_USEMILITARYTIME			= "Utiliser le format 24 heures";
--MONKEYBUDDY_CLOCK_LOCK						= "Lock the MonkeyClock frame";
--MONKEYBUDDY_CLOCK_CHATALARM					= "Use the alarm message in the chat window";
--MONKEYBUDDY_CLOCK_DIALOGALARM				= "Use the alarm dialog box with snooze button";
MONKEYBUDDY_CLOCK_ALLOWRIGHTCLICK			= "Clic droit pour ouvrir MonkeyBuddy";

MONKEYBUDDY_CLOCK_HOURSLIDER				= "RÃ©glage Heure";
MONKEYBUDDY_CLOCK_MINUTESLIDER				= "RÃ©glage Minute";

--MONKEYBUDDY_CLOCK_ALARMHOURSLIDER			= "Alarm Hour";
--MONKEYBUDDY_CLOCK_ALARMMINUTESLIDER			= "Alarm Minute";

-- bindings
BINDING_NAME_MONKEYBUDDY_OPEN				= "Ouvrir/Fermer la fenÃªtre de configuration";

end

if (GetLocale() == "zhTW") then

-- English, the default
MONKEYBUDDY_DESCRIPTION						= "幫助你對MonkeyMods進行設定";
MONKEYBUDDY_LOADED_MSG						= MONKEYBUDDY_INFO_COLOUR .. MONKEYBUDDY_TITLE .. " v" .. MONKEYBUDDY_VERSION .. " 載入";

MONKEYBUDDY_TOOLTIP_CLOSE					= "關閉";
MONKEYBUDDY_RESET_ALL						= "全部重設";
MONKEYBUDDY_RESET_TO_BLIZZARD_STYLE			= "暴雪風格";
MONKEYBUDDY_RESET							= "重設";

MONKEYBUDDY_GUI_TEXT						="這個插件是幫助你對MonkeyMods（猴子插件模組）進行設定的工具"
MONKEYBUDDY_GUI_MBLOADED					= MONKEYBUDDY_GUI_ICON .. "MonkeyBuddy成功載入！"
MONKEYBUDDY_GUI_MMINSTALLED					="啟用的MonkeyMods："
MONKEYBUDDY_GUI_MBMINIMAP					="顯示MonkeyBuddy的小地圖圖示"
MONKEYBUDDY_GUI_MQEXTRA						="MonkeyQuest額外功能："
MONKEYBUDDY_GUI_MQEDAILY					="在大標題顯示每日任務的計數器"
MONKEYBUDDY_GUI_MBOPENCFG					="點擊按鈕開啟"

-- defs for MonkeyQuest
MONKEYBUDDY_QUEST_OPEN						= "開啟MonkeyQuest";
MONKEYBUDDY_QUEST_SHOWHIDDEN				= "顯示所有隱藏的項目";
MONKEYBUDDY_QUEST_USEOVERVIEWS				= "沒有具體的任務目標時，顯示任務概述";
MONKEYBUDDY_QUEST_HIDEHEADERS				= "不顯示隱藏的項目時，一併隱藏區域標題";
MONKEYBUDDY_QUEST_ALWAYSHEADERS				= "總是顯示區域標題";
MONKEYBUDDY_QUEST_HIDEBORDER				= "隱藏邊框";
MONKEYBUDDY_QUEST_GROWUP					= "向上展開";
MONKEYBUDDY_QUEST_SHOWNUMQUESTS				= "顯示任務數量";
MONKEYBUDDY_QUEST_LOCK						= "鎖定MonkeyQuest視窗";
MONKEYBUDDY_QUEST_COLOURTITLEON				= "根據難度著色任務標題";
MONKEYBUDDY_QUEST_HIDECOMPLETEDQUESTS		= "隱藏已完成任務";
MONKEYBUDDY_QUEST_HIDECOMPLETEDOBJECTIVES	= "隱藏已完成目標";
MONKEYBUDDY_QUEST_SHOWTOOLTIPOBJECTIVES		= "在滑鼠提示中顯示目標進度";
MONKEYBUDDY_QUEST_ALLOWRIGHTCLICK			= "右鍵點擊視窗開啟MonkeyBuddy";
MONKEYBUDDY_QUEST_HIDETITLEBUTTONS			= "隱藏主標題按鈕";
MONKEYBUDDY_QUEST_HIDETITLE					= "隱藏主標題的「" .. MONKEYBUDDY_QUEST_TITLE .. "」文字";
MONKEYBUDDY_QUEST_FONT						= "字型：";
MONKEYBUDDY_QUEST_CRASHBORDER				= "使用金色邊框";
MONKEYBUDDY_QUEST_SHOWNOOBTIPS				= "顯示新手提示";
MONKEYBUDDY_QUEST_SHOWZONEHIGHLIGHT			= "顯示任務區域高亮";
MONKEYBUDDY_QUEST_SHOWQUESTLEVEL			= "顯示任務等級";
MONKEYBUDDY_QUEST_WORKCOMPLETE				= "啟用「任務完成」的音效";
MONKEYBUDDY_QUEST_COLOURSUBOBJECTIVES		= "根據進度著色任務目標";
MONKEYBUDDY_QUEST_ITEMSENABLED				= "顯示任務物品按鈕（6.0.3後會汙染暴雪元素）"
MONKEYBUDDY_QUEST_ITEMSONLEFT				= "將任務物品顯示在左側";
MONKEYBUDDY_QUEST_HIDEHEADER				= "只在滑鼠指向時顯示大標題和和大標題按鈕"
MONKEYBUDDY_QUEST_HIDEQUESTSENABLED			= "右鍵點擊任務隱藏；只在未勾選「顯示所有隱藏的項目」時生效"
MONKEYBUDDY_QUEST_COLOURDONEORFAILED		= "著色已完成或失敗的任務標題"
MONKEYBUDDY_QUEST_QUESTTEXTTOOLTIP			= "滑鼠指向時，顯示完整的任務文本"

MONKEYBUDDY_QUEST_QUESTTITLECOLOUR			= "任務標題";
MONKEYBUDDY_QUEST_HEADEROPENCOLOUR			= "展開的區域標題";
MONKEYBUDDY_QUEST_HEADERCLOSEDCOLOUR		= "折疊的區域標題";
MONKEYBUDDY_QUEST_OVERVIEWCOLOUR			= "任務概述";
MONKEYBUDDY_QUEST_SPECIALOBJECTIVECOLOUR	= "特殊目標";
MONKEYBUDDY_QUEST_INITIALOBJECTIVECOLOUR	= "目標進度0%";
MONKEYBUDDY_QUEST_MIDOBJECTIVECOLOUR		= "目標進度50%以下";
MONKEYBUDDY_QUEST_COMPLETEOBJECTIVECOLOUR	= "目標進度50%以上";
MONKEYBUDDY_QUEST_FINISHOBJECTIVECOLOUR		= "完成的目標";
MONKEYBUDDY_QUEST_ZONEHIGHLIGHTCOLOUR		= "高亮目前區域";

MONKEYBUDDY_QUEST_FRAMEALPHASLIDER			= "全局透明度";
MONKEYBUDDY_QUEST_ALPHASLIDER				= "背景透明度";
MONKEYBUDDY_QUEST_HIGHLIGHTALPHASLIDER		= "區域高亮透明度";
MONKEYBUDDY_QUEST_WIDTHSLIDER				= "視窗寬度";
MONKEYBUDDY_QUEST_FONTSLIDER				= "字型大小";
MONKEYBUDDY_QUEST_PADDINGSLIDER				= "任務間距";

-- defs for MonkeySpeed
MONKEYBUDDY_SPEED_OPEN						= "開啟MonkeySpeed";
MONKEYBUDDY_SPEED_PERCENT					= "以百分比顯示速度";
MONKEYBUDDY_SPEED_BAR						= "Show speed as a background colour";
MONKEYBUDDY_SPEED_LOCK						= "鎖定MonkeySpeed視窗";
MONKEYBUDDY_SPEED_ALLOWRIGHTCLICK			= "允許右鍵點擊開啟MonkeyBuddy";

MONKEYBUDDY_SPEED_WIDTHSLIDER				= "視窗寬度";
MONKEYBUDDY_SPEED_HEIGHTSLIDER				= "視窗高度";

-- defs for MonkeyClock
MONKEYBUDDY_CLOCK_OPEN						= "開啟MonkeyClock";
MONKEYBUDDY_CLOCK_HIDEBORDER				= "隱藏邊框";
MONKEYBUDDY_CLOCK_USEMILITARYTIME			= "使用24小時制";
MONKEYBUDDY_CLOCK_LOCK						= "Lock the MonkeyClock frame";
MONKEYBUDDY_CLOCK_CHATALARM					= "Use the alarm message in the chat window";
MONKEYBUDDY_CLOCK_DIALOGALARM				= "Use the alarm dialog box with snooze button";
MONKEYBUDDY_CLOCK_ALLOWRIGHTCLICK			= "Allow right click to open MonkeyBuddy";

MONKEYBUDDY_CLOCK_HOURSLIDER				= "Hour Offset";
MONKEYBUDDY_CLOCK_MINUTESLIDER				= "Minute Offset";

MONKEYBUDDY_CLOCK_ALARMHOURSLIDER			= "Alarm Hour";
MONKEYBUDDY_CLOCK_ALARMMINUTESLIDER			= "Alarm Minute";

-- bindings
BINDING_NAME_MONKEYBUDDY_OPEN				= "開啟控制台";

elseif (GetLocale() == "zhCN") then

-- English, the default
MONKEYBUDDY_DESCRIPTION						= "帮助你对MonkeyMods进行配置";
MONKEYBUDDY_LOADED_MSG						= MONKEYBUDDY_INFO_COLOUR .. MONKEYBUDDY_TITLE .. " v" .. MONKEYBUDDY_VERSION .. " 已加载";

MONKEYBUDDY_TOOLTIP_CLOSE					= "关闭";
MONKEYBUDDY_RESET_ALL						= "全部重设";
MONKEYBUDDY_RESET_TO_BLIZZARD_STYLE			= "暴雪风格";
MONKEYBUDDY_RESET							= "重设";

MONKEYBUDDY_GUI_TEXT						="这个插件是帮助你对MonkeyMods进行配置的工具"
MONKEYBUDDY_GUI_MBLOADED					= MONKEYBUDDY_GUI_ICON .. "MonkeyBuddy成功加载！"
MONKEYBUDDY_GUI_MMINSTALLED					="启用的MonkeyMods："
MONKEYBUDDY_GUI_MBMINIMAP					="显示MonkeyBuddy的小地图图标"
MONKEYBUDDY_GUI_MQEXTRA						="MonkeyQuest额外功能："
MONKEYBUDDY_GUI_MQEDAILY					="在大标题显示日常任务计数器"
MONKEYBUDDY_GUI_MBOPENCFG					="点击按钮开启"

-- defs for MonkeyQuest
MONKEYBUDDY_QUEST_OPEN						= "开启MonkeyQuest";
MONKEYBUDDY_QUEST_SHOWHIDDEN				= "显示所有隐藏的项目";
MONKEYBUDDY_QUEST_USEOVERVIEWS				= "没有具体的任务目标时，显示任务概述";
MONKEYBUDDY_QUEST_HIDEHEADERS				= "不显示隐藏的项目时，一并隐藏区域标题";
MONKEYBUDDY_QUEST_ALWAYSHEADERS				= "总是显示区域标题";
MONKEYBUDDY_QUEST_HIDEBORDER				= "隐藏边框";
MONKEYBUDDY_QUEST_GROWUP					= "向上展开";
MONKEYBUDDY_QUEST_SHOWNUMQUESTS				= "显示任务数量";
MONKEYBUDDY_QUEST_LOCK						= "锁定MonkeyQuest";
MONKEYBUDDY_QUEST_COLOURTITLEON				= "根据难度着色任务标题";
MONKEYBUDDY_QUEST_HIDECOMPLETEDQUESTS		= "隐藏已完成任务";
MONKEYBUDDY_QUEST_HIDECOMPLETEDOBJECTIVES	= "隐藏已完成目标";
MONKEYBUDDY_QUEST_SHOWTOOLTIPOBJECTIVES		= "在鼠标提示中显示目标进度";
MONKEYBUDDY_QUEST_ALLOWRIGHTCLICK			= "右键点击开启MonkeyBuddy";
MONKEYBUDDY_QUEST_HIDETITLEBUTTONS			= "隐藏主标题按钮";
MONKEYBUDDY_QUEST_HIDETITLE					= "隐藏主标题的“" .. MONKEYBUDDY_QUEST_TITLE .. "”文字";
MONKEYBUDDY_QUEST_FONT						= "字型：";
MONKEYBUDDY_QUEST_CRASHBORDER				= "使用金色边框";
MONKEYBUDDY_QUEST_SHOWNOOBTIPS				= "显示新手提示";
MONKEYBUDDY_QUEST_SHOWZONEHIGHLIGHT			= "显示任务区域高亮";
MONKEYBUDDY_QUEST_SHOWQUESTLEVEL			= "显示任务等级";
MONKEYBUDDY_QUEST_WORKCOMPLETE				= "启用“任务完成”的音效";
MONKEYBUDDY_QUEST_COLOURSUBOBJECTIVES		= "根据进度着色任务目标";
MONKEYBUDDY_QUEST_ITEMSENABLED				= "显示任务物品按钮（6.0.3后会污染暴雪元素）"
MONKEYBUDDY_QUEST_ITEMSONLEFT				= "将任务物品显示在左侧";
MONKEYBUDDY_QUEST_HIDEHEADER				= "只在鼠标指向时显示大标题和和大标题按钮"
MONKEYBUDDY_QUEST_HIDEQUESTSENABLED			= "右键点击任务隐藏；只在未勾选“显示所有隐藏的项目”时生效"
MONKEYBUDDY_QUEST_COLOURDONEORFAILED		= "着色已完成或失败的任务标题"
MONKEYBUDDY_QUEST_QUESTTEXTTOOLTIP			= "鼠标指向时，显示完整的任务文本"

MONKEYBUDDY_QUEST_QUESTTITLECOLOUR			= "任务标题";
MONKEYBUDDY_QUEST_HEADEROPENCOLOUR			= "展开的区域标题";
MONKEYBUDDY_QUEST_HEADERCLOSEDCOLOUR		= "折叠的区域标题";
MONKEYBUDDY_QUEST_OVERVIEWCOLOUR			= "任务概述";
MONKEYBUDDY_QUEST_SPECIALOBJECTIVECOLOUR	= "特殊目标";
MONKEYBUDDY_QUEST_INITIALOBJECTIVECOLOUR	= "目标进度0%";
MONKEYBUDDY_QUEST_MIDOBJECTIVECOLOUR		= "目标进度50%以下";
MONKEYBUDDY_QUEST_COMPLETEOBJECTIVECOLOUR	= "目标进度50%以上";
MONKEYBUDDY_QUEST_FINISHOBJECTIVECOLOUR		= "完成的目标";
MONKEYBUDDY_QUEST_ZONEHIGHLIGHTCOLOUR		= "高亮目前区域";

MONKEYBUDDY_QUEST_FRAMEALPHASLIDER			= "全局透明度";
MONKEYBUDDY_QUEST_ALPHASLIDER				= "背景透明度";
MONKEYBUDDY_QUEST_HIGHLIGHTALPHASLIDER		= "区域高亮透明度";
MONKEYBUDDY_QUEST_WIDTHSLIDER				= "窗口宽度";
MONKEYBUDDY_QUEST_FONTSLIDER				= "字号";
MONKEYBUDDY_QUEST_PADDINGSLIDER				= "任务间距";

-- defs for MonkeySpeed
MONKEYBUDDY_SPEED_OPEN						= "启用MonkeySpeed";
MONKEYBUDDY_SPEED_PERCENT					= "显示百分比";
MONKEYBUDDY_SPEED_BAR						= "以移动速度染色背景";
MONKEYBUDDY_SPEED_LOCK						= "锁定MonkeySpeed视窗";
MONKEYBUDDY_SPEED_ALLOWRIGHTCLICK			= "右键点击开启MonkeyBuddy";

MONKEYBUDDY_SPEED_WIDTHSLIDER				= "视窗宽度";
MONKEYBUDDY_SPEED_HEIGHTSLIDER				= "视窗高度";

-- defs for MonkeyClock
MONKEYBUDDY_CLOCK_OPEN						= "启用MonkeyClock";
MONKEYBUDDY_CLOCK_HIDEBORDER				= "隐藏边框";
MONKEYBUDDY_CLOCK_USEMILITARYTIME			= "使用24小时制";
MONKEYBUDDY_CLOCK_LOCK						= "锁定窗口";
MONKEYBUDDY_CLOCK_CHATALARM					= "在聊天框提醒闹钟响了";
MONKEYBUDDY_CLOCK_DIALOGALARM				= "U弹出对话框提醒闹钟响了";
MONKEYBUDDY_CLOCK_ALLOWRIGHTCLICK			= "右键点击开启配置选项";

MONKEYBUDDY_CLOCK_HOURSLIDER				= "小时偏移";
MONKEYBUDDY_CLOCK_MINUTESLIDER				= "分钟偏移";

MONKEYBUDDY_CLOCK_ALARMHOURSLIDER			= "定闹钟的时";
MONKEYBUDDY_CLOCK_ALARMMINUTESLIDER			= "设定闹钟的分";

-- bindings
BINDING_NAME_MONKEYBUDDY_OPEN				= "开启控制台";

end

if (GetLocale() == "ruRU") then

MONKEYBUDDY_DESCRIPTION						= "Помогает настроить MonkeyMods.";
MONKEYBUDDY_LOADED_MSG						= MONKEYBUDDY_INFO_COLOUR .. MONKEYBUDDY_TITLE .. " v" .. MONKEYBUDDY_VERSION .. " загружен";

MONKEYBUDDY_TOOLTIP_CLOSE					= "Закрыть";
MONKEYBUDDY_RESET_ALL						= "Сбросить все";
MONKEYBUDDY_RESET_TO_BLIZZARD_STYLE			= "Стиль Blizzard";
MONKEYBUDDY_RESET							= "Сброс";

MONKEYBUDDY_GUI_TEXT						="Этот аддон позволяет легко настроить свой MonkeyMods"
MONKEYBUDDY_GUI_MBLOADED					= MONKEYBUDDY_GUI_ICON .. "MonkeyBuddy успешно загружен!"
MONKEYBUDDY_GUI_MMINSTALLED					="Установлен MonkeyMods:"
MONKEYBUDDY_GUI_MBMINIMAP					="Показать значок на миникарте MonkeyBuddy"
MONKEYBUDDY_GUI_MQEXTRA						="MonkeyQuest Extra:"
MONKEYBUDDY_GUI_MQEDAILY					="Показывать счетчик ежедневных заданий в заголовке"
MONKEYBUDDY_GUI_MBOPENCFG					="Нажмите следующую кнопку, чтобы открыть"

-- defs for MonkeyQuest
MONKEYBUDDY_QUEST_OPEN						= "Открыть MonkeyQuest";
MONKEYBUDDY_QUEST_SHOWHIDDEN				= "Показать скрытые предметы";
MONKEYBUDDY_QUEST_USEOVERVIEWS				= "Используйте обзоры, когда нет целей";
MONKEYBUDDY_QUEST_HIDEHEADERS				= "Скрыть заголовки зон, если не отображаются скрытые предметы";
MONKEYBUDDY_QUEST_ALWAYSHEADERS				= "Всегда показывать заголовки зон, всегда";
MONKEYBUDDY_QUEST_HIDEBORDER				= "Скрыть границу";
MONKEYBUDDY_QUEST_GROWUP					= "Расширять вверх";
MONKEYBUDDY_QUEST_SHOWNUMQUESTS				= "Показать количество квестов";
MONKEYBUDDY_QUEST_LOCK						= "Заблокируйте окно MonkeyQuest";
MONKEYBUDDY_QUEST_COLOURTITLEON				= "Раскрасьте названия заданий по сложности";
MONKEYBUDDY_QUEST_HIDECOMPLETEDQUESTS		= "Скрыть выполненные задания";
MONKEYBUDDY_QUEST_HIDECOMPLETEDOBJECTIVES	= "Скрыть выполненные цели";
MONKEYBUDDY_QUEST_SHOWTOOLTIPOBJECTIVES		= "Показывать объективную полноту во всплывающих подсказках";
MONKEYBUDDY_QUEST_ALLOWRIGHTCLICK			= "Разрешить ПКМ, чтобы открыть MonkeyBuddy";
MONKEYBUDDY_QUEST_HIDETITLEBUTTONS			= "Скрыть кнопки заголовков";
MONKEYBUDDY_QUEST_HIDETITLE					= "Скрыть заголовок (" .. MONKEYBUDDY_QUEST_TITLE .. ") текст";
MONKEYBUDDY_QUEST_FONT						= "Шрифт:";
MONKEYBUDDY_QUEST_CRASHBORDER				= "Используйте золотой цвет границы";
MONKEYBUDDY_QUEST_SHOWNOOBTIPS				= "Показать полезные подсказки для новичков";
MONKEYBUDDY_QUEST_SHOWZONEHIGHLIGHT			= "Показать выделение зоны задания";
MONKEYBUDDY_QUEST_SHOWQUESTLEVEL			= "Показать уровни заданий";
MONKEYBUDDY_QUEST_WORKCOMPLETE				= "Включить >работа завершена< звук";
MONKEYBUDDY_QUEST_COLOURSUBOBJECTIVES		= "Цвет цели по прогрессу";
MONKEYBUDDY_QUEST_ITEMSENABLED				= "Показать кнопки предметов заданий (испорчено в 6.0.3)"
MONKEYBUDDY_QUEST_ITEMSONLEFT				= "Показать предметы заданий слева";
MONKEYBUDDY_QUEST_HIDEHEADER				= "Показывать только заголовок/кнопки заголовка при наведении курсора мыши"
MONKEYBUDDY_QUEST_HIDEQUESTSENABLED			= "Скрыть задания с помощью ПКМ, если не отображаются скрытые предметы"
MONKEYBUDDY_QUEST_COLOURDONEORFAILED		= "Цвет завершенных или неудачных заданий"
MONKEYBUDDY_QUEST_QUESTTEXTTOOLTIP			= "Показать полный текст заданий при наведении курсора"

MONKEYBUDDY_QUEST_QUESTTITLECOLOUR			= "Заголовки заданий";
MONKEYBUDDY_QUEST_HEADEROPENCOLOUR			= "Заголовки открытых зон";
MONKEYBUDDY_QUEST_HEADERCLOSEDCOLOUR		= "Заголовки закрытых зон";
MONKEYBUDDY_QUEST_OVERVIEWCOLOUR			= "Обзоры заданий";
MONKEYBUDDY_QUEST_SPECIALOBJECTIVECOLOUR	= "Особые цели";
MONKEYBUDDY_QUEST_INITIALOBJECTIVECOLOUR	= "Задания на 0%";
MONKEYBUDDY_QUEST_MIDOBJECTIVECOLOUR		= "Задания на 50%";
MONKEYBUDDY_QUEST_COMPLETEOBJECTIVECOLOUR	= "Задания на <100%";
MONKEYBUDDY_QUEST_FINISHOBJECTIVECOLOUR		= "Завершенные задания";
MONKEYBUDDY_QUEST_ZONEHIGHLIGHTCOLOUR		= "Текущее выделение зоны";

MONKEYBUDDY_QUEST_FRAMEALPHASLIDER			= "Глобальная прозрачность";
MONKEYBUDDY_QUEST_ALPHASLIDER				= "Прозрачность Фона";
MONKEYBUDDY_QUEST_HIGHLIGHTALPHASLIDER		= "Прозрачность выделения зоны";
MONKEYBUDDY_QUEST_WIDTHSLIDER				= "Ширина окна";
MONKEYBUDDY_QUEST_FONTSLIDER				= "Размер шрифта";
MONKEYBUDDY_QUEST_PADDINGSLIDER				= "Наполнение задания";

-- defs for MonkeySpeed
MONKEYBUDDY_SPEED_OPEN						= "Открыть MonkeySpeed";
MONKEYBUDDY_SPEED_PERCENT					= "Показывать скорость в процентах";
MONKEYBUDDY_SPEED_BAR						= "Показывать скорость как цвет фона";
MONKEYBUDDY_SPEED_LOCK						= "Заблокируйте окно MonkeySpeed";
MONKEYBUDDY_SPEED_ALLOWRIGHTCLICK			= "Разрешить ПКМ, чтобы открыть MonkeyBuddy";

MONKEYBUDDY_SPEED_WIDTHSLIDER				= "Ширина окна";
MONKEYBUDDY_SPEED_HEIGHTSLIDER				= "Высота окна";

-- defs for MonkeyClock
MONKEYBUDDY_CLOCK_OPEN						= "Открыть MonkeyClock";
MONKEYBUDDY_CLOCK_HIDEBORDER				= "Скрыть границу";
MONKEYBUDDY_CLOCK_USEMILITARYTIME			= "Использовать 24-часовой формат";
MONKEYBUDDY_CLOCK_LOCK						= "Заблокируйте окно MonkeyClock";
MONKEYBUDDY_CLOCK_CHATALARM					= "Используйте тревожное сообщение в окне чата";
MONKEYBUDDY_CLOCK_DIALOGALARM				= "Используйте диалоговое окно будильника с кнопкой повтора";
MONKEYBUDDY_CLOCK_ALLOWRIGHTCLICK			= "Разрешить ПКМ, чтобы открыть MonkeyBuddy";

MONKEYBUDDY_CLOCK_HOURSLIDER				= "Настройка часов";
MONKEYBUDDY_CLOCK_MINUTESLIDER				= "Настройка минут";

MONKEYBUDDY_CLOCK_ALARMHOURSLIDER			= "Будильник";
MONKEYBUDDY_CLOCK_ALARMMINUTESLIDER			= "Секундомер";

-- bindings
BINDING_NAME_MONKEYBUDDY_OPEN				= "Открыть/закрыть окно настроек";

end